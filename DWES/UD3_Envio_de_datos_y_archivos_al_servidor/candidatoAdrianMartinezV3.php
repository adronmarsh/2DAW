<?php
    ...
    if (!empty($_POST)) { //Este código se ejecutará una vez enviado el formulario
        ...
        if (!empty($_FILES)) { //Comprueba los posibles errores de los ficheros
            ...
            if ($tipoCurriculum!='application/pdf'){
                $errores['tipoCurriculum']=$errorTipoCurriculum;
            }
            else{
                if (is_uploaded_file($directorioTempCurriculum) === true){
...
<html lang="es">
    ...
    <body>
        ...
        <?php
            ...
            if (!empty($errores)) { //Si hay errores muestra el formulario indicando los errores.
                ?>
                <form name="datos" action="#" method="POST" enctype="multipart/form-data">
            ...
            if(!empty($_POST&&empty($errores))) { //Muestra los resultados
