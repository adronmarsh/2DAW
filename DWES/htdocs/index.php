  ...
  <main>
    <div class="container">
      ...
      <?php
        ...
        $revels = $conexion->query("SELECT * FROM revels ORDER BY fecha");
        foreach ($revels->fetchAll() as $revel) { //Recorre la tabla revels
          if ($revel['userid'] == $userid) {
            $revelid = $revel['id'];
            ...
            echo $revel['texto'] . '<br>';
            echo $revel['fecha'] . '<br>';
            $comentarios = $conexion->query("SELECT * FROM comments WHERE revelid = $userid");
            $count = 0;
        ...
        foreach ($follows->fetchAll() as $follow) { //Recorre la tabla follows
          ...
          foreach ($revels->fetchAll() as $revel) { //Recorre la tabla revels
            if ($revel['userid'] == $userid) {
              ...
              echo $revel['texto'] . '<br>';
              echo $revel['fecha'] . '<br>';
              $comentarios = $conexion->query("SELECT * FROM comments WHERE revelid = $followedid");
              $count = 0;
              foreach ($comentarios->fetchAll() as $comentario) {
                $count++;
              }
